...
本文件是 `other_projects/cherry/net/nats/` 的局部索引。进入本目录工作时，先读本文件，再按需读取相邻源码、测试或上级文档。
...
- Go package：`cherryNats`。
- 本目录索引用于快速定位本 package 的文件与下级目录，具体行为以源码和测试为准。

## 主要文件

- [`connect.go`](connect.go)
- [`connect_pool.go`](connect_pool.go)
- [`msg_pool.go`](msg_pool.go)
- [`msg_pool_test.go`](msg_pool_test.go)
- [`req_id.go`](req_id.go)
- [`timer_pool.go`](timer_pool.go)
