...
本文件是 `other_projects/cherry/extend/time_wheel/` 的局部索引。进入本目录工作时，先读本文件，再按需读取相邻源码、测试或上级文档。
...
- Go package：`cherryTimeWheel`。
- 本目录索引用于快速定位本 package 的文件与下级目录，具体行为以源码和测试为准。

## 主要文件

- [`bucket.go`](bucket.go)
- [`delay_queue.go`](delay_queue.go)
- [`schedule.go`](schedule.go)
- [`time_wheel.go`](time_wheel.go)
- [`time_wheel_test.go`](time_wheel_test.go)
- [`timer.go`](timer.go)
- [`utils.go`](utils.go)
