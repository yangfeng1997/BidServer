...
本文件是 `other_projects/cherry/extend/snowflake/` 的局部索引。进入本目录工作时，先读本文件，再按需读取相邻源码、测试或上级文档。
...
- Go package：`cherrySnowflake`。
- 本目录索引用于快速定位本 package 的文件与下级目录，具体行为以源码和测试为准。

## 主要文件

- [`snowflake.go`](snowflake.go)
- [`snowflake_default.go`](snowflake_default.go)
- [`snowflake_test.go`](snowflake_test.go)
