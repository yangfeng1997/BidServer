...
本文件是 `other_projects/ProjectBid/serialize/` 的局部索引。进入本目录工作时，先读本文件，再按需读取相邻源码、测试或上级文档。
...
- [`json_serializer.go`](json_serializer.go)
- [`protobuf_serializer.go`](protobuf_serializer.go)
- [`serializer.go`](serializer.go)
- [`serializer_test.go`](serializer_test.go)
