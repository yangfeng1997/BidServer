...
func runAsync(d taskqueue.TaskQueue, op func() error, done func(error)) {
	go func() {
		err := op()
...
func (c *Client) FindByID(d taskqueue.TaskQueue, coll string, id any, out any, done func(found bool, err error)) {
	found := false
...
func (c *Client) UpsertSetByID(d taskqueue.TaskQueue, coll string, id any, set bson.M, done func(error)) {
	runAsync(d, func() error {
...
func (c *Client) InsertOne(d taskqueue.TaskQueue, coll string, doc any, done func(insertedID any, err error)) {
	var insertedID any
	runAsync(d, func() error {
...
func (c *Client) Find(d taskqueue.TaskQueue, coll string, filter bson.M, sort bson.D, limit int64, out any, done func(error)) {
	runAsync(d, func() error {
		opt := options.Find()
		if len(sort) > 0 {
			opt.SetSort(sort)
...
func (c *Client) UpdateByID(d taskqueue.TaskQueue, coll string, id any, update bson.M, upsert bool, done func(error)) {
	runAsync(d, func() error {
...
func (c *Client) FindOneAndUpdate(d taskqueue.TaskQueue, coll string, filter bson.M, update bson.M, returnUpdated bool, out any, done func(found bool, err error)) {
	found := false
	runAsync(d, func() error {
