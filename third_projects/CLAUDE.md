...
本文件是 `third_projects/` 的总入口索引，目标是让你**最快**定位到某个参考项目的根文档、热点模块和关键源码。
...
- 当我修改某个参考项目的代码，且改动会影响该目录的结构、入口、配置、生命周期、命令、信号、热更语义或生成链路时，我会同步更新该项目以及必要的上级 `CLAUDE.md`。
- 如果只是纯实现细节的小修补，通常不改文档；但只要会改变索引、加载顺序或工程约定，我会补写。
...
3. 子项目源码和测试优先于文档。
4. 本文件只管 `third_projects/` 的总边界和最快加载顺序。
5. 不明确时先问，不要默默猜。
