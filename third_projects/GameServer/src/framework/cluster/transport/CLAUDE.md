...
本文件是 `third_projects/GameServer/src/framework/cluster/transport/` 的局部索引。进入本目录工作时，先读本文件，再按需读取相邻源码、测试或上级文档。
...
- [`nats_cluster.go`](nats_cluster.go)
- [`nats_rpc.go`](nats_rpc.go)
- [`nats_rpc_test.go`](nats_rpc_test.go)
