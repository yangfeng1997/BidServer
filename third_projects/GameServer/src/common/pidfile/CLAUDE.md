...
本文件是 `third_projects/GameServer/src/common/pidfile/` 的局部索引。进入本目录工作时，先读本文件，再按需读取相邻源码、测试或上级文档。
...
- [`isrunning_unix.go`](isrunning_unix.go)
- [`isrunning_windows.go`](isrunning_windows.go)
- [`pidfile.go`](pidfile.go)
- [`pidfile_test.go`](pidfile_test.go)
