...
本文件是 `third_projects/GameServer/src/common/daemon/` 的局部索引。进入本目录工作时，先读本文件，再按需读取相邻源码、测试或上级文档。
...
- [`daemon.go`](daemon.go)
- [`daemon_test.go`](daemon_test.go)
- [`daemonize_unix.go`](daemonize_unix.go)
- [`daemonize_windows.go`](daemonize_windows.go)
