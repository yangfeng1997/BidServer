...
本文件是 `third_projects/GameServer/src/servers/matchsvr/internal/` 的局部索引。进入本目录工作时，先读本文件，再按需读取相邻源码、测试或上级文档。
...
- [`match_consumer.go`](match_consumer.go)
- [`match_consumer_test.go`](match_consumer_test.go)
- [`match_integration_test.go`](match_integration_test.go)
- [`match_module.go`](match_module.go)
- [`queue.go`](queue.go)
