...
本文件是 `third_projects/GameServer/src/servers/routersvr/internal/` 的局部索引。进入本目录工作时，先读本文件，再按需读取相邻源码、测试或上级文档。
...
- [`router_handler.go`](router_handler.go)
- [`router_handler_test.go`](router_handler_test.go)
- [`router_integration_test.go`](router_integration_test.go)
- [`router_module.go`](router_module.go)
