...
本文件是 `third_projects/GameServer/src/servers/gatesvr/internal/` 的局部索引。进入本目录工作时，先读本文件，再按需读取相邻源码、测试或上级文档。
...
- [`gate_handler.go`](gate_handler.go)
- [`gate_handler_test.go`](gate_handler_test.go)
- [`gate_module.go`](gate_module.go)
- [`kick_test.go`](kick_test.go)
- [`push_test.go`](push_test.go)
