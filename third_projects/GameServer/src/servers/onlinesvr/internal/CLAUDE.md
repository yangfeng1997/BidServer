...
本文件是 `third_projects/GameServer/src/servers/onlinesvr/internal/` 的局部索引。进入本目录工作时，先读本文件，再按需读取相邻源码、测试或上级文档。
...
- [`directory_test.go`](directory_test.go)
- [`online_handler.go`](online_handler.go)
- [`online_handler_test.go`](online_handler_test.go)
- [`online_integration_test.go`](online_integration_test.go)
- [`online_module.go`](online_module.go)
