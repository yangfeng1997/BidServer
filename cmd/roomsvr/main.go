...
func execute() error {
	...
	if opts.RoomConfigPath == "" {
		return fmt.Errorf("room config path is required")
	}
	if opts.NodeID == "" {
	...
	if opts.Daemon {
		...
		if err != nil {
			return fmt.Errorf("start roomsvr daemon: %w", err)
		}
		if started {
			return nil
		}
	...
	builder := room.NewRoomBuilder(room.Options{
		...
		RoomConfigPath: opts.RoomConfigPath,
	})

	app, err := builder.Build()
	if err != nil {
		return fmt.Errorf("build roomsvr app: %w", err)
	}
	if err := process.WritePIDFile(opts.PidFile); err != nil {
		return fmt.Errorf("write roomsvr pid file: %w", err)
	}
	defer func() {
		if err := process.RemovePIDFile(opts.PidFile); err != nil {
			fmt.Fprintf(os.Stderr, "remove roomsvr pid file: %v\n", err)
		}
	}()
...
func bindFlags(opts *room.Options) {
	pflag.StringVarP(&opts.PidFile, "pid-file", "p", "roomsvr.pid", "pid file path")
	...
	pflag.StringVar(&opts.RoomConfigPath, "room-config", "", "room config path")
