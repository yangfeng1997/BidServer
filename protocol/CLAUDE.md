...
- 协议 proto 源文件与生成代码。
- `common/` 放协议注解、错误码和节点类型；`cs/` 放客户端协议；`handler/` 放前端 handler 服务；`service/` 放后端 RPC 服务；`ra/` 放 routeragent 协议；`gen/` 放生成的路由、handler、service 和 RPC stub。
...
- 查后端 RPC 先看 `service/` 和 `gen/rpc.go`。
