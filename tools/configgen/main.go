...
func main() {
	...
	if err := os.WriteFile(filepath.Join(outDir, "entry.go"), []byte(renderEntry(messages)), 0o644); err != nil {
		fmt.Fprintf(os.Stderr, "write entry.go: %v\n", err)
		os.Exit(1)
	}
...
func renderEntry(messages []messageInfo) string {
