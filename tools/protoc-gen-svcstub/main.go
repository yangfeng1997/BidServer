...
func parseMethods(f *protogen.File, s *protogen.Service, kind serviceKind, protoText string) ([]methodSpec, error) {
	...
	for _, m := range s.Methods {
		...
		if cmdID == "" {
			cmdID = "0" // 无 cmd_id 时默认为 0（Remote 类无 cmd_id，FRONTEND 必须有）
		}
...
func writeServiceFile(p *protogen.Plugin, spec serviceSpec, dir string) error {
	g := p.NewGeneratedFile(filepath.Join(dir, spec.FileBaseName+"_"+dir+".go"), protogen.GoImportPath("project/protocol/gen/"+dir))
	...
	for _, m := range spec.Methods {
		...
		if m.Notify {
			g.P("\t// ", route, " (Notify)")
			g.P("\td.MustRegister(", routeConst, ", corerpc.RecoverRoute(", routeConst, ", func(ctx corerpc.Ctx, body []byte, reply func([]byte, error)) error {")
